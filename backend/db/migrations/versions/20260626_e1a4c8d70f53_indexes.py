...
def upgrade() -> None:
    ...
    op.execute(
        "CREATE INDEX ix_production_output_work_order_id ON production_output (work_order_id);"
    )
    ...
    op.execute(
        "CREATE INDEX ix_quality_inspections_work_order_id ON quality_inspections (work_order_id);"
    )
